...
| Project | What it is |
|---|---|
| `Backend/` | The whole app: a self-hostable comment backend for static blogs, plus the admin console that manages it. It has its own `AGENTS.md`; read it before touching anything in there. |
...
AppHost, `npm run dev` is a `vite build --watch`, not a dev server. A browser
refresh picks up a rebuild.
...
`POST /api/site`, not a redeploy. The console itself needs no entry because it is
...
Components run in this process and call the services directly: no HTTP, no DTOs,
no client-side JavaScript. `Login.razor` and `Register.razor` declare no render
mode on purpose: a cookie can only be written during a real form POST, which an
interactive circuit does not give you. Do not add a render mode to them, and do
...
**Backend loads the solution-root `.env` relative to the working directory.** It
looks for `.env` then `../.env`, so it works under `dotnet run --project Backend`
and AppHost (project directory as CWD) and in Docker, where the file is absent
and `docker-compose.yml` passes the same keys as real environment variables.
...
`X-Forwarded-Proto`. The tunnel must pass WebSockets because the console is
...
- Tailwind utilities live in markup. `Styles/app.css` holds only what Blazor's
  own class names (`.invalid`, `.validation-message`, `.blazor-error-boundary`)
  force into CSS, plus the `h1:focus` rule `<FocusOnNavigate>` needs.
- Vite entry is `Styles/main.js`: CSS only, no application JavaScript. Output is
  `wwwroot/dist` with stable filenames; Blazor's `MapStaticAssets` and
  `@Assets[...]` do the fingerprinting.
- Prefer CSS-only interactions where they work. Destructive actions confirm
  inline with component state rather than a JS `confirm()`.
- `BareLayout` is for unauthenticated full-screen pages. `MainLayout` is for the
